...
def cat_boxlist_with_keypoints(boxlists):
    assert all(boxlist.has_field("bb8keypoints") for boxlist in boxlists)

    kp = [boxlist.get_field("bb8keypoints").keypoints for boxlist in boxlists]

    kp = cat(kp, 0)

    fields = boxlists[0].get_fields()
    fields = [field for field in fields if field != "bb8keypoints"]

    boxlists = [boxlist.copy_with_fields(fields) for boxlist in boxlists]
    boxlists = cat_boxlist(boxlists)
    boxlists.add_field("bb8keypoints", kp)
    return boxlists
...
class BB8KeypointRCNNLossComputation(object):
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            keypoints_per_image = matched_targets.get_field("bb8keypoints")
    ...
    def subsample(self, proposals, targets):
        ...
        for labels_per_image, keypoints_per_image, proposals_per_image in zip(
            ...
            proposals_per_image.add_field("bb8keypoints", keypoints_per_image)
    ...
    def __call__(self, proposals, keypoint_logits):
        ...
        for proposals_per_image in proposals:
            kp = proposals_per_image.get_field("bb8keypoints")
...
def make_roi_bb8keypoint_loss_evaluator(cfg):
    matcher = Matcher(
    ...
    resolution = cfg.MODEL.ROI_BB8KEYPOINT_HEAD.RESOLUTION
    loss_evaluator = BB8KeypointRCNNLossComputation(matcher, fg_bg_sampler, resolution)
