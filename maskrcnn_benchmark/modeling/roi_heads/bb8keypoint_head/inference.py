...
class BB8KeypointPostProcessor(nn.Module):
    def __init__(self, keypointer=None):
        super(BB8KeypointPostProcessor, self).__init__()
    ...
    def forward(self, x, boxes):
        ...
        for prob, box, score in zip(mask_prob, boxes, scores):
            ...
            prob = BB8Keypoints(prob, box.size)
            prob.add_field("logits", score)
            bbox.add_field("bb8keypoints", prob)
            results.append(bbox)
...
def make_roi_bb8keypoint_post_processor(cfg):
    keypointer = BB8Keypointer()
    keypoint_post_processor = BB8KeypointPostProcessor(keypointer)
